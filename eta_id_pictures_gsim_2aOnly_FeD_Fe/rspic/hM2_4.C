{
//=========Macro generated from canvas: c/The canvas
//=========  (Tue Dec 20 14:49:17 2016) by ROOT version5.34/19
   TCanvas *c = new TCanvas("c", "The canvas",1,1,800,576);
   gStyle->SetOptFit(1);
   c->SetHighLightColor(2);
   c->Range(0.425,-0.7941257,0.675,1.814224);
   c->SetBorderSize(2);
   c->SetLogy();
   c->SetFrameFillColor(0);
   
   TH1F *hM = new TH1F("hM","Mass bin (pt2,nu,z) = (0,0,4)",75,0.45,0.65);
   hM->SetBinContent(0,756);
   hM->SetBinContent(1,6);
   hM->SetBinContent(2,6);
   hM->SetBinContent(3,14);
   hM->SetBinContent(4,15);
   hM->SetBinContent(5,9);
   hM->SetBinContent(6,4);
   hM->SetBinContent(7,3);
   hM->SetBinContent(8,3);
   hM->SetBinContent(9,5);
   hM->SetBinContent(10,8);
   hM->SetBinContent(11,5);
   hM->SetBinContent(12,3);
   hM->SetBinContent(13,9);
   hM->SetBinContent(14,6);
   hM->SetBinContent(15,4);
   hM->SetBinContent(16,4);
   hM->SetBinContent(17,5);
   hM->SetBinContent(18,4);
   hM->SetBinContent(19,9);
   hM->SetBinContent(20,5);
   hM->SetBinContent(21,6);
   hM->SetBinContent(22,10);
   hM->SetBinContent(23,3);
   hM->SetBinContent(24,1);
   hM->SetBinContent(25,9);
   hM->SetBinContent(26,7);
   hM->SetBinContent(27,2);
   hM->SetBinContent(28,4);
   hM->SetBinContent(29,6);
   hM->SetBinContent(30,1);
   hM->SetBinContent(31,6);
   hM->SetBinContent(32,6);
   hM->SetBinContent(33,5);
   hM->SetBinContent(34,5);
   hM->SetBinContent(35,4);
   hM->SetBinContent(36,8);
   hM->SetBinContent(37,9);
   hM->SetBinContent(38,7);
   hM->SetBinContent(39,6);
   hM->SetBinContent(40,6);
   hM->SetBinContent(41,3);
   hM->SetBinContent(42,9);
   hM->SetBinContent(43,6);
   hM->SetBinContent(44,4);
   hM->SetBinContent(45,11);
   hM->SetBinContent(46,7);
   hM->SetBinContent(47,2);
   hM->SetBinContent(48,6);
   hM->SetBinContent(49,4);
   hM->SetBinContent(50,5);
   hM->SetBinContent(51,8);
   hM->SetBinContent(52,8);
   hM->SetBinContent(53,8);
   hM->SetBinContent(54,5);
   hM->SetBinContent(55,11);
   hM->SetBinContent(56,7);
   hM->SetBinContent(57,11);
   hM->SetBinContent(58,11);
   hM->SetBinContent(59,6);
   hM->SetBinContent(60,9);
   hM->SetBinContent(61,4);
   hM->SetBinContent(62,1);
   hM->SetBinContent(63,6);
   hM->SetBinContent(64,11);
   hM->SetBinContent(65,8);
   hM->SetBinContent(66,8);
   hM->SetBinContent(67,4);
   hM->SetBinContent(68,6);
   hM->SetBinContent(69,5);
   hM->SetBinContent(70,8);
   hM->SetBinContent(71,7);
   hM->SetBinContent(72,9);
   hM->SetBinContent(73,5);
   hM->SetBinContent(74,3);
   hM->SetBinContent(75,13);
   hM->SetBinContent(76,35852);
   hM->SetEntries(37085);
   
   TPaveStats *ptstats = new TPaveStats(0.62,0.515,0.98,0.995,"brNDC");
   ptstats->SetName("stats");
   ptstats->SetBorderSize(2);
   ptstats->SetTextAlign(12);
   TText *text = ptstats->AddText("hM");
   text->SetTextSize(0.04014545);
   text = ptstats->AddText("Entries = 37085  ");
   text = ptstats->AddText("Mean  = 0.5527");
   text = ptstats->AddText("RMS   = 0.05994");
   text = ptstats->AddText("#chi^{2} / ndf = 124.7 / 69");
   text = ptstats->AddText("p0       = 232.6 #pm 0.2 ");
   text = ptstats->AddText("p1       = -194.5 #pm 0.5 ");
   text = ptstats->AddText("p2       = 180.7 #pm 0.8 ");
   text = ptstats->AddText("p3       = -176.2 #pm 0.2 ");
   text = ptstats->AddText("p4       = -3.171e+05 #pm 4.938e+02 ");
   text = ptstats->AddText("p5       = 4.062e+13 #pm 1.414e+00 ");
   ptstats->SetOptStat(1111);
   ptstats->SetOptFit(111);
   ptstats->Draw();
   hM->GetListOfFunctions()->Add(ptstats);
   ptstats->SetParent(hM);
   
   TF1 *fsig = new TF1("fsig","fmb+fpeak",0.45,0.65);
   fsig->SetNpx(500);
   fsig->SetFillColor(19);
   fsig->SetFillStyle(0);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#ff0000");
   fsig->SetLineColor(ci);
   fsig->SetLineWidth(3);
   fsig->SetChisquare(124.6529);
   fsig->SetNDF(69);
   fsig->GetXaxis()->SetLabelFont(42);
   fsig->GetXaxis()->SetLabelSize(0.035);
   fsig->GetXaxis()->SetTitleSize(0.035);
   fsig->GetXaxis()->SetTitleFont(42);
   fsig->GetYaxis()->SetLabelFont(42);
   fsig->GetYaxis()->SetLabelSize(0.035);
   fsig->GetYaxis()->SetTitleSize(0.035);
   fsig->GetYaxis()->SetTitleFont(42);
   fsig->SetParameter(0,232.6471);
   fsig->SetParError(0,0.2484055);
   fsig->SetParLimits(0,0,0);
   fsig->SetParameter(1,-194.5064);
   fsig->SetParError(1,0.451965);
   fsig->SetParLimits(1,0,0);
   fsig->SetParameter(2,180.6976);
   fsig->SetParError(2,0.8062837);
   fsig->SetParLimits(2,0,0);
   fsig->SetParameter(3,-176.2339);
   fsig->SetParError(3,0.2484055);
   fsig->SetParLimits(3,0,0);
   fsig->SetParameter(4,-317052);
   fsig->SetParError(4,493.816);
   fsig->SetParLimits(4,0,0);
   fsig->SetParameter(5,4.061716e+13);
   fsig->SetParError(5,1.414214);
   fsig->SetParLimits(5,0,0);
   hM->GetListOfFunctions()->Add(fsig);

   ci = TColor::GetColor("#0000ff");
   hM->SetMarkerColor(ci);
   hM->SetMarkerStyle(24);
   hM->GetXaxis()->SetTitle("M_{#gamma#gamma} (GeV)");
   hM->GetXaxis()->SetLabelFont(42);
   hM->GetXaxis()->SetLabelSize(0.035);
   hM->GetXaxis()->SetTitleSize(0.035);
   hM->GetXaxis()->SetTitleFont(42);
   hM->GetYaxis()->SetTitle("dN/dM (GeV)");
   hM->GetYaxis()->SetLabelFont(42);
   hM->GetYaxis()->SetLabelSize(0.035);
   hM->GetYaxis()->SetTitleSize(0.035);
   hM->GetYaxis()->SetTitleFont(42);
   hM->GetZaxis()->SetLabelFont(42);
   hM->GetZaxis()->SetLabelSize(0.035);
   hM->GetZaxis()->SetTitleSize(0.035);
   hM->GetZaxis()->SetTitleFont(42);
   hM->Draw("e");
   
   TF1 *fmb = new TF1("fmb","pol2",0.54,0.56);
   fmb->SetFillColor(19);
   fmb->SetFillStyle(0);

   ci = TColor::GetColor("#00ff00");
   fmb->SetLineColor(ci);
   fmb->SetLineWidth(3);
   fmb->SetLineStyle(2);
   fmb->GetXaxis()->SetLabelFont(42);
   fmb->GetXaxis()->SetLabelSize(0.035);
   fmb->GetXaxis()->SetTitleSize(0.035);
   fmb->GetXaxis()->SetTitleFont(42);
   fmb->GetYaxis()->SetLabelFont(42);
   fmb->GetYaxis()->SetLabelSize(0.035);
   fmb->GetYaxis()->SetTitleSize(0.035);
   fmb->GetYaxis()->SetTitleFont(42);
   fmb->SetParameter(0,232.6471);
   fmb->SetParError(0,0);
   fmb->SetParLimits(0,0,0);
   fmb->SetParameter(1,-194.5064);
   fmb->SetParError(1,0);
   fmb->SetParLimits(1,0,0);
   fmb->SetParameter(2,180.6976);
   fmb->SetParError(2,0);
   fmb->SetParLimits(2,0,0);
   fmb->Draw("same");
   
   TF1 *fpeak = new TF1("fpeak","gaus",0.54,0.56);
   fpeak->SetFillColor(19);
   fpeak->SetFillStyle(0);
   fpeak->SetLineWidth(3);
   fpeak->SetLineStyle(2);
   fpeak->GetXaxis()->SetLabelFont(42);
   fpeak->GetXaxis()->SetLabelSize(0.035);
   fpeak->GetXaxis()->SetTitleSize(0.035);
   fpeak->GetXaxis()->SetTitleFont(42);
   fpeak->GetYaxis()->SetLabelFont(42);
   fpeak->GetYaxis()->SetLabelSize(0.035);
   fpeak->GetYaxis()->SetTitleSize(0.035);
   fpeak->GetYaxis()->SetTitleFont(42);
   fpeak->SetParameter(0,-176.2339);
   fpeak->SetParError(0,0);
   fpeak->SetParLimits(0,0,0);
   fpeak->SetParameter(1,-317052);
   fpeak->SetParError(1,0);
   fpeak->SetParLimits(1,0,0);
   fpeak->SetParameter(2,4.061716e+13);
   fpeak->SetParError(2,0);
   fpeak->SetParLimits(2,0,0);
   fpeak->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.9404546,0.4709548,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(2);
   text = pt->AddText("Mass bin (pt2,nu,z) = (0,0,4)");
   pt->Draw();
   c->Modified();
   c->cd();
   c->SetSelected(c);
}
