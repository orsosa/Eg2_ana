{
  TFile *outf = new TFile("gammECorr.root","recreate");
  TFile *file0 = TFile::Open("pi0_CD_2aonly_c.root");
  TNtuple *t = new TNtuple("t","t","E:cf");
  ...
  outdata->SetAlias("M_c","TMath::Sqrt(2*E1c*E2c*(1.-cos(alpha)))");
  outdata->SetAlias("E1tc","E1/(1.129-0.05793/E1 + 1.07e-12/E1/E1)");
  outdata->SetAlias("E2tc","E2/(1.129-0.05793/E2 + 1.07e-12/E2/E2)");
  //  outdata->SetAlias("M_c","TMath::Sqrt(2*E1/*E2c*(1.-cos(alpha)))");
  outdata->SetAlias("M_tc","TMath::Sqrt(2*E1tc*E2tc*(1.-cos(alpha)))");
  ...
  fm->SetParameter(0,1.);
  TF1 *ft = new TF1("ft","1.129-0.05793/x + 1.07e-12/x/x",0.35,1.2); //C correction
  Float_t NBins=50.0;
  Float_t Emin=0.35;
  Float_t Emax=1.2;
  ...
  outf->cd();
  hcfm->Draw("same");
  hcfm->Write("",TObject::kOverwrite);
  outf->Close();
  outdata->Draw("M_tc>>h(500,0,1)");
