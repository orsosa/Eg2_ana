...
int gamma_sfHE(Float_t all=0,Float_t ahl=1)
{
  TFile *file0 = TFile::Open("/data/user/o/orsosa/osoto_ana/pi0_CD_2aonly_c.root");
  ...
  */
  ...
  TCut E1c_cut=Form("0.4<E1c&&E1c<0.5");
  Float_t Emin=1.4,Emax=2.5;
  Int_t NBE2=7;
  Float_t DE=(Emax-Emin)/NBE2;

  for (int k=0;k<NBE2;k++)
  {
    Float_t Ell=Emin+k*DE;
    Float_t Ehl=Emin+(k+1.)*DE;
    TCut E2_cut=Form("%f<E2&&E2<%f",Ell,Ehl);

    outdata->Draw("E1c:E2>>h(500,1.4,2.5,500,0.4,0.5)",alpha_cut0&&mass_cut&&E1c_cut&&E2_cut,"goff");
    TH2F * h = (TH2F*)gROOT->FindObject("h");
    TF1 *fm = new TF1 ("fm",Form("0.1349766*0.1349766/(4.*TMath::Sin(%f*TMath::DegToRad()/2)*TMath::Sin(%f*TMath::DegToRad()/2)*x/[0])",(all+ahl)/2.,(all+ahl)/2.),1.4,2.5);
    h->Fit("fm","RQ");
    Float_t E = h->GetMean(2);
