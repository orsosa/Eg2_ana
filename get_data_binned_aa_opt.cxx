...
int main(int argc, char *argv[])
{
  ...
  minRange = 0., maxRange = 1; ///// range of plots.
  //Float_t rangeFit[2]={.35,.75}; //only eta fit
  Float_t rangeFit[2]={0.05,.75}; //full range  fit
 ...
 //////// Workspaces and datasets needed //////////
  RooWorkspace **ws_arr;
  RooDataSet **ds_arr;
  RooRealVar **m_arr;
  ws_arr = new RooWorkspace *[Nbins];
  ds_arr = new RooDataSet *[Nbins];
  m_arr= new RooRealVar *[Nbins];;
  for (int k=0;k<Nbins;k++) 
  {
    m_arr[k]= new RooRealVar(Form("x%d",k),"Invariant Mass",0.1,0.,1.,"GeV");
    ws_arr[k] = new RooWorkspace(Form("ws_bin%d",k),"RooFit Workspace");
    ws_arr[k] = new RooWorkspace(Form("ws_bin%d",k),"RooFit Workspace");
    RooDataSet *ds = new RooDataSet("ds","data set",m_arr[k]);
  }
  /////////////////////////////////////////////////////////
...
/////// fit invariant mass spectrum //////////
    //RooPlot* frame=fit_pdf_aa(hM, minRange, maxRange, err, Ns, Nb, ds,&meta, 1, 0.55,0.045,w,rangeFit,bkg_model); // fit considering background shape.
    //Neta=3000.;Nb=3000.;
    RooPlot* frame=fit_pdf_aa(hM, minRange, maxRange, err, Neta, Nb, ds,&meta, 1, 0.55,0.045,w,rangeFit); // fit everithing on eta range.
    //    RooPlot* frame=fit_pdf_aa(hM, minRange, maxRange, err, Ns, Nb, ds,&meta, 1, 0.55,0.045,w,rangeFit);// no background shape a priori.
...
/////////////////////////////////////////////
    ...
    Float_t mbratio=Neta.getVal()/Npi0.getVal();
