...
int main(int argc, char *argv[]){
  ...
  TNtuple *tgs = (TNtuple *) f2->Get("tuplePi0_gamma");
  ...
  TFile *fout = new TFile(Form("%s/pi0_eta_sim_%sD_%s_gamma_binned.root",indir,st,tt),"recreate");
  tuple_rec1 = new TNtuple("trec1","reconstructed gamma (Q2,Nu,Z)","M:Nu:Q2:Z:Pt2:Event:bin:binq2:binnu:binz");
  tuple_gsim1 = new TNtuple("tgsim1","reconstructed gamma (Q2,Nu,Z)","M:Nu:Q2:Z:Pt2:Event:bin:binq2:binnu:binz");

  tuple_rec2 = new TNtuple("trec2","reconstructed gamma (Nu,z,Pt2)","M:Nu:Q2:Z:Pt2:Event:bin:binpt2:binnu:binz");
  tuple_gsim2 = new TNtuple("tgsim2","thrown gamma (Nu,z,Pt2)","M:Nu:Q2:Z:Pt2:Event:bin:binpt2:binnu:binz");
  ...
  for (int k = 0;k <Nbins; k++)
  {
    ...
    {
      tuple_rec1->Fill(M[i],Nu[i],Q2[i],Z[i],Pt2[i],Event[i],k,binq2,binnu,binz);
    }

    size = tgs->Draw("M:Nu:Q2:Z:Pt2:Event",Form("%f<Q2&&Q2<%f && %f<Nu&&Nu<%f && %f<Z&&Z<%f",Q2l,Q2h,Nul,Nuh,Zl,Zh),"goffcandle");
    M=tgs->GetVal(0);
    Nu=tgs->GetVal(1);
    Q2=tgs->GetVal(2);
    Z=tgs->GetVal(3);
    ...
    for (int i=0;i<size;i++)
    {
      tuple_gsim1->Fill(M[i],Nu[i],Q2[i],Z[i],Pt2[i],Event[i],k,binq2,binnu,binz);
    }
...
//binning (Pt2,Nu,Z)
  ...
  for (int k = 0;k <Nbins; k++)
  {
    ...
    for (int i=0;i<size;i++)
    {
      tuple_rec2->Fill(M[i],Nu[i],Q2[i],Z[i],Pt2[i],Event[i],k,binpt2,binnu,binz);
    }

    size = tgs->Draw("M:Nu:Q2:Z:Pt2:Event",Form("%f<Pt2&&Pt2<%f && %f<Nu&&Nu<%f && %f<Z&&Z<%f",Pt2l,Pt2h,Nul,Nuh,Zl,Zh),"goffcandle");
    M=tgs->GetVal(0);
    Nu=tgs->GetVal(1);
    Q2=tgs->GetVal(2);
    Z=tgs->GetVal(3);
    ...
    for (int i=0;i<size;i++)
    {
      tuple_gsim2->Fill(M[i],Nu[i],Q2[i],Z[i],Pt2[i],Event[i],k,binpt2,binnu,binz);
    }
