...
Float_t kMPi0=5.47E-01;
Float_t kSPi0=0.1*5.98542e-02;
Float_t kPt2,kEvent; 
...
int get_bin_info()
{
  char *bfile = Form("binning_info.txt");
  if (stat(bfile, &sb) != 0)
  {
  ...
  reset_array(b);
  while(bfile.good())
  while(bfile>>h>>b[0]>>b[1]>>b[2]>>b[3]>>h>>b[4]>>b[5]>>b[6]>>b[7]>>h>>b[8]>>b[9])
...
int main(int argc, char *argv[]){
  ...
  //  Float_t minRange = 0.13, maxRange=0.14;
  Float_t minRange = 0.54, maxRange=0.56;
  ...
  tuple1 = new TNtuple("tuple1","recsim gsim Pi0 (Q2,Nu,Z)","M:Nu:Q2:Z:Pt2:Event:bin:binq2:binnu:binz");
  tuple2 = new TNtuple("tuple2","recsim gsim Pi0 (Nu,z,Pt2)","M:Nu:Q2:Z:Pt2:Event:bin:binpt2:binnu:binz");

  tuple1s = new TNtuple("tuple1s","gsim with mbratio (Q2,Nu,Z)","Nu:Q2:Z:Pt2:amp:bin:binq2:binnu:binz:mbratio");
  tuple2s = new TNtuple("tuple2s","gsim with mbratio (Nu,z,Pt2)","Nu:Q2:Z:Pt2:amp:bin:binpt2:binnu:binz:mbratio");
  ...
  for (int k = 0;k <Nbins; k++)
  {
    ...
    fsig->SetParameter(3,hM->GetMaximum()*0.9);
    fsig->SetParameter(4,kMPi0);
    fsig->SetParameter(5,kSPi0);
    fsig->SetNpx(500);
    hM->Draw("e");
    TFitResultPtr rp = hM->Fit("fsig","Qs");
    if (rp->Chi2()/rp->Ndf() > 30)
    {  
      
      hM->Fit("fsig","Qs");
    }
    Float_t totI = fsig->Integral(minRange,maxRange);
    hM->Write(Form("/hM1_%d.gif",k),TObject::kOverwrite);
...
//binning (Pt2,Nu,Z)
  ...
  for (int k = 0;k <Nbins; k++)
  {
    ...
    if (rp->Chi2()/rp->Ndf() > 30)
      hM->Fit("fsig","+Qs");
    Float_t totI = fsig->Integral(minRange,maxRange);
    hM->Write(Form("/hM2_%d.gif",k),TObject::kOverwrite);
    fmb->SetParameter(0,fsig->GetParameter(0));
    fmb->SetParameter(1,fsig->GetParameter(1)); 
    fmb->SetParameter(2,fsig->GetParameter(2));
    fpeak->SetParameter(0,fsig->GetParameter(3));
